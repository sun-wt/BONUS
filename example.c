...
int32_t cmd_hello(struct cli_def *cli, const char *command, char *argv[], int argc) 
{
    cli_print(cli, "hello world");
    return 0;
}
int32_t cmd_test(struct cli_def *cli, const char *command, char *argv[], int argc) 
{
	cli_print(cli, "called %s with %s\r\n", __func__, command);
	return CLI_OK;
}

int32_t cmd_set(struct cli_def *cli, const char *command, char *argv[], int argc) 
{
  if (argc < 2) 
	{
